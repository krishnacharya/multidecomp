...
class BuildGroupwise_noniid:
    def __init__(self, df:pd.DataFrame, target_col:str, sort_col:str, A_t : pd.DataFrame, ascending = True):
        '''
        df: dataframe minmax numeric, one hot categoric
        df must have all features then column for y
        target_col : regression target in the dataframe (y)
        sort_col : column by which to sort the dataframe, then the sequence we observe is not iid
    ...
    def build_all_seeds(self, l2_pen:float = 1.0):
        ...
        self.base_obj_list = []
        self.Anh_obj_list = []
        self.mergesort_indices = [] # sorted indices for each sorted shuffled(with randseed) dataframe
        for seed in self.rand_seeds:
            # first we shuffle both df, and A_t by the same seed, then sort df_shuf according to the sort col e.g. age (find the indices)
            df_shuf = self.df.sample(frac = 1, random_state = seed)
            A_t_shuf = self.A_t.sample(frac = 1, random_state = seed)
            sort_ind = np.argsort(df_shuf[self.sort_col], kind='mergesort') # default is ascending, also mergesort preserves original order
            if not self.ascending: 
                sort_ind = sort_ind[::-1] # make descending ordering
            self.mergesort_indices.append(sort_ind)
            df_shuf = df_shuf.iloc[sort_ind]
            A_t_shuf_np = A_t_shuf.iloc[sort_ind].to_numpy()
            X_dat_np = df_shuf.drop(self.target_col, axis = 1).to_numpy()
            y_dat_np = df_shuf[self.target_col].to_numpy()
    ...
    def build_regret_curve(self):
        ...
        for ind in range(self.num_runs): # corresponding b_Anh has the Anh obj for that random seed
            ...
            df_shuf = self.df.sample(frac=1, random_state = seed).iloc[self.mergesort_indices[ind]]
            A_t_shuf = self.A_t.sample(frac=1, random_state = seed).iloc[self.mergesort_indices[ind]] # dataframe
            X_dat_shuf = df_shuf.drop(self.target_col, axis = 1)
            y_dat_shuf = df_shuf[self.target_col]
        
            for g_ind, gname in enumerate(self.group_names):
                indices_g = (A_t_shuf[gname] == 1) # group indicator indices
...
def plot_subgroups(gwise_obj: BuildGroupwise_noniid, subgroups_list : list):
    '''
...
def plot_regret_curve_with_std(gwise_obj: BuildGroupwise_noniid, dir_name:str):
...
def get_end_regret_gw_df(gwise_obj: BuildGroupwise_noniid) -> pd.DataFrame:
    df_rows = []
    for g_ind, gname in enumerate(gwise_obj.group_names):
        gwise_obj.regret_Anh_groupwise_array[g_ind] = np.array(gwise_obj.regret_Anh_groupwise_array[g_ind]) # all 10 values in the row have same dim, so can make np array
        gwise_obj.regret_Base_groupwise_array[g_ind] = np.array(gwise_obj.regret_Base_groupwise_array[g_ind])
